---
layout: publication
title: "A New Non-MDS Hash Function Resisting Birthday Attack and Meet-in-the-middle
Attack"
...
as only one block into a digest of m bits, where 80 <= m <= 232 and 80 <= m <= n
<= 4096. The analysis and proof show that the new hash is one-way, weakly
...
birthday attack is given. Compared with the Chaum-Heijst-Pfitzmann hash based on
a discrete logarithm problem, the new hash is lightweight, and thus it opens a
door to convenience for utilization of lightweight digital signing schemes.
