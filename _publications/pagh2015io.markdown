...
classical algorithms includes a factor of \$N^\rho\$, where \$\rho\$ is a
parameter of the LSH used, the I/O complexity of our algorithm merely includes a
factor \$(N/M)^\rho\$, where \$N\$ is the data size and \$M\$ is the size of
internal memory. Our algorithm is randomized and outputs the correct result with
high probability. It is a simple, recursive, cache-oblivious procedure, and we
